...
from states.state_payment import StorageQiwi, StorageCtyproBot
from utils import send_all_admin, clear_firstname
from utils.crypto_bot import Crypto, Executor as CryptoExecutor
###################################################################################
############################## ВВОД СУММЫ ПОПОЛНЕНИЯ ##############################
# Выбор способа пополнения
from utils.db_api.sqlite import update_userx, get_refillx, add_refillx, add_invoice, del_invoice
...
async def input_amount(call: CallbackQuery):
    ...
    await StorageCtyproBot.here_input_crypro_bot.set()
    '''
    check_pass = False
...
@dp.message_handler(state = StorageCtyproBot.here_input_crypro_bot)
async def create_crypto_bot_pay(message: types.Message, state: FSMContext):
    ...
    min_input = 1  # Минимальная сумма пополнения в $лях

    if pay_amount < min_input:
        await bot.delete_message(message.chat.id, del_msg.message_id)
        return await message.answer(
            f"❌ <b>Неверная сумма пополнения</b>\n"
            f"▶ Мин. сумма пополнения: <code>{min_input}$</code>\n"
            f"💵 Введите сумму для пополнения средств 🤖"
        )

    fiat_invoice = await CryptoExecutor.crypto.create_invoice(
        amount = pay_amount,
        asset = 'USDT'
    )
    ...
    await message.answer(
        ...
        "html",
        reply_markup = asser_crypto_markup(fiat_invoice)   
    )
    await state.finish()
...
async def create_qiwi_pay(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        pay_amount = int(message.text)
        del_msg = await bot.send_message(message.from_user.id, "<b>♻ Подождите, платёж генерируется...</b>")
        min_input_qiwi = 1  # Минимальная сумма пополнения в $лях
...
async def check_qiwi_pay(call: CallbackQuery):
    ...
    if get_payments[0] != "None" or get_payments[1] != "None" or get_payments[2] != "None":
        ...
        pay_amount = float(qiwi.check(bill_id=receipt).amount)  # Получение суммы платежа в $лях
            ...
            for check_pay in get_history:
                if str(receipt) == str(check_pay["comment"]):
                    if "643" == str(check_pay["sum"]["currency"]):
                        ...
                        pay_amount = float(check_pay["sum"]["amount"])  # Получение суммы платежа в $лях
                        pay_amount = int(pay_amount)
                    else:
                        await bot.answer_callback_query(call.id, "❗ Оплата была произведена не в $лях.", True)
            if pay_status:
                get_purchase = await get_refillx("*", receipt=receipt)
                if get_purchase is None:
